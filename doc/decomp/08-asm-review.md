...
### (b) TU on both sides
```bash
find build/split -type f -name '*.o' -print0 | xargs -0 $NM -A 2>/dev/null | grep -w "$SYM"
# build/split/nu2api/nucore/deflate.cpp.o:00000588 T _Z19DecodeDeflatedBlockP14DEFLATECONTEXT
...
```bash
jq -r '.units[] | select(.name == "nu2api/nucore/deflate.cpp.o" or (.name | test("deflate"))) | [.name, .target_path, .base_path] | @tsv' objdiff.json
# nu2api/nucore/deflate.cpp.o	build/split/nu2api/nucore/deflate.cpp.o	/home/fabian/git/saga/build/CMakeFiles/saga.dir/src/nu2api/nucore/deflate.cpp.o
```
Unit names preserve the object path below `CMakeFiles/saga.dir/src` and include
the `.o` suffix: `src/nu2api/nucore/deflate.cpp` becomes
`nu2api/nucore/deflate.cpp.o`. `base_path` carries the full recompiled path. If
recursive `nm -A` on the recompiled side is empty, the
...
numbering as the objdiff instruction addresses (§4). 100.0 = done, skip. Four
current units (`globals.cpp.o`, `nu2api/nu3d/nutexanm.c.o`,
`nu2api/nufile/android/NuFileAndroidDeviceAPK.cpp.o`, and
`nu2api/nusound/nusound.cpp.o`) have `functions == null` (nothing paired
into them) — guard with `?`/`? //`. The `ogg_vorbis` extra_units unit is *not* one
of them — it has 36 functions like a normal TU.
Note: committed `report.json` can lag the current tree; root `report.md` is an
ignored historical artifact. Use `objdiff-cli report generate` for live truth;
numbers here are for triage only.

### (d) Classified mismatch list (objdiff-cli diff + jq)
One-shot JSON dump to stdout, then straight into jq — never eyeball it raw:
```bash
objdiff-cli diff -1 build/split/nu2api/nucore/deflate.cpp.o -2 build/CMakeFiles/saga.dir/src/nu2api/nucore/deflate.cpp.o -o - > /tmp/xl.json
# one-shot JSON contains the whole unit; filter the selected symbol below
```
...
$OJ -d build/split/nu2api/nucore/deflate.cpp.o > /tmp/orig.asm
...
- **left = `-1` = target** = the carved original under `build/split/`. **right = `-2` = base** = the recompiled `build/CMakeFiles/saga.dir/**/*.o`. The side on `-1` is what `match_percent` is computed *against* and instruction alignment is directional—keep split on `-1` always.
...
- **The `<symbol>` argument resolves/selects a unit, but one-shot JSON still
  contains the whole unit.** Every jq view must carry
  `select(.name == $sym)`. Pass `--arg sym "$sym"`.
...
- **`functions == null` units** (currently 4, §3c): guard optional arrays with
  `?`; there are no
...
- **`-p <project> -u <unit>` alternative**: `objdiff-cli diff -p . -u nu2api/nucore/deflate.cpp.o -o -` reads the paths out of `objdiff.json`. Copy the exact unit name—including `.o`—rather than guessing it.
...
# TU + unit name (for -p/-u): nu2api/nucore/deflate.cpp.o — copied from objdiff.json
xasm() { awk "/^[0-9a-f]+ <$1>:/,/^\$/" "$2" | awk 'NF' | tail -n +2 | sed -E 's/^[[:space:]]*[0-9a-f]+:[[:space:]]*//; s/[[:space:]]+/ /g; s/[[:space:]]+$//; s/ [0-9a-f]+ (<[^>]+>)/ \1/g'; }
...
objdiff-cli diff -1 build/split/nu2api/nucore/deflate.cpp.o \
...
```bash
objdiff-cli diff -p . -u nu2api/nucore/deflate.cpp.o -o - > /tmp/xl.json
