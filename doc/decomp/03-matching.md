...
- **target** = original code carved into a source-shaped object tree under `build/split/`.
...
Output preserves the object path below `CMakeFiles/saga.dir/src`, for example
`build/split/legoapi/core/input/qrand.cpp.o`. Parent directories are created
by gonk, so the non-recursive `build/split/*.o` glob is incomplete.
...
TU unit (`used_symbols`) and not ignored. Target = `build/split/remaining.c.o`,
base = the empty `remaining.c.o` at the repository root. The current report
attributes 391,472 code bytes, 1,515 functions, and 12,294,420 data bytes to
this unit. No `scratch` block → objdiff can never pair anything → no fuzzy keys anywhere
...
`fuzzy_match_percent`). It never gains matched bytes.

**Unit naming**: the source-relative object path is used verbatim, including
the `.o` suffix—for example `gameapi/ai/aisys/aiscript.cpp.o` and
`batman.cpp.o`. Renaming or moving a source file changes its unit name and
target path (see §7).

## 3. objdiff.json

`objdiff.json` (regenerated by `gonk split`) is the objdiff project config,
currently 466 units (464 compile-command TUs, `ogg_vorbis`, and `remaining`):
...
`report.json` top-level `measures` (2026-08-29 snapshot):
`fuzzy_match_percent` 9.302378, `total_code` 4,758,621,
`matched_code` 83,738, `total_functions` 13,623,
`matched_functions` 1,012, and `total_units` 466. Per unit:
...
`matched_code` and `matched_functions` count functions with
`fuzzy_match_percent` **exactly 100.0**. So
...
now carries a warning banner. Neither current `objdiffdiff.py` (stdout) nor
`objdiff-cli report changes` (JSON) produces it; do not use it for progress.
...
- Compiler-generated `.isra`, `.constprop`, `.part`, `.cold`,
  `_GLOBAL__sub_I_`, and `_GLOBAL__sub_D_` names are reported separately and
  excluded from the must-provide set.
...
**undefined extern reloc targets** in carved objects. Search recursively under
`build/split/` to find those references; the ignore filter
...
1. **Adding/renaming/merging source files moves symbols between units → pairing churn
   with zero code change.** Unit names are source-relative `.o` paths; gonk
   re-pairs by symbol name and objdiffdiff joins by name. A file rename can
   therefore cause churn even if bytes are identical. Check whether source
   changed and whether objdiffdiff reports a moved unit. The verdict fields
...
2. **Duplicate same-name functions across files.** The original's
   ...
   defining the same function can carve the *same* original symbol → one function appears
   in multiple units (86 duplicate function names in the current report); objdiffdiff's
...
8. **`remaining` can't be fuzzy-matched.** Its base is empty and has no
   scratch config, so its functions never contribute to matched/fuzzy metrics;
   they only count toward `total_code`, `total_functions`, and `total_data`.
   Matched progress requires
...
#   the symbol selects/resolves the unit, but one-shot JSON contains the whole
#   unit — filter with jq (08 §3d/§5)
#   -p <project> -u <unit> resolves an exact `.o`-suffixed unit from objdiff.json
...
python3 scripts/objdiffdiff.py report.json report_old.json   # stdout is Markdown; don't overwrite historical report.md
...
source (`objdiff-cli` is installed from `ttdecomp/objdiff` via Cargo):
...
- **Report level**: a function is counted in `matched_code`/`matched_functions` iff
  ...
  `measures.matched_code`; functions between 99 and 100 are excluded.
  objdiffdiff.py uses its own FULL_MATCH = 99.999 threshold for
